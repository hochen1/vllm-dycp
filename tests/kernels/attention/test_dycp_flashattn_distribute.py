...
def run_distributed_fa(rank, world_size, case):
    ...
    # lse_local_f32 = lse_local.to(torch.float32)
    
    # exp_lse = torch.exp(lse_local_f32).permute(1, 0).unsqueeze(-1) 
    
    # numerator_local = out_local.to(torch.float32) * exp_lse
    # denominator_local = exp_lse

    # dist.all_reduce(numerator_local, op=dist.ReduceOp.SUM)
    # dist.all_reduce(denominator_local, op=dist.ReduceOp.SUM)

    # out_dist = (numerator_local / (denominator_local + 1e-12)).to(dtype)
