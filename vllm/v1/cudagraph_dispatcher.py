...
class CudagraphDispatcher:
    ...
    def initialize_cudagraph_keys(
        self, cudagraph_mode: CUDAGraphMode, uniform_decode_query_len: int, num_dcp_tokens: int
    ):
        ...
        if (
            ...
            num_dcp_tokens_list = [i for i in range(num_dcp_tokens)]
            for bs, has_lora, cp_tokens in product(cudagraph_capture_sizes_for_decode, lora_cases, num_dcp_tokens_list):
                self.add_cudagraph_key(
                    CUDAGraphMode.FULL,
                    self._create_padded_batch_descriptor(bs, True, has_lora, cp_tokens),
                )
    ...
    def dispatch(
        ...
        batch_desc = self._create_padded_batch_descriptor(
            num_tokens, uniform_decode, has_lora, num_dcp_tokens
        )
        ...
        if not disable_full:
            ...
            if batch_desc in self.cudagraph_keys[CUDAGraphMode.FULL]:
                print(f"not disable_full: {batch_desc}", flush=True)
