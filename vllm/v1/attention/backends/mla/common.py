...
def reorg_kvcache(
    ...
    if len(kv_c_segments) == 0:
        ...
        raise ValueError(
            ...
            f"chunk_idx={chunk_idx}, toks={toks}"
        )
...
class MLACommonImpl(MLACommonBaseImpl[M], Generic[M]):
    ...
    def _resolve_dycp_chunked_context_consensus(
        ...
        # DyCP collectives require all ranks to take the same branch. Do not
        # silently downgrade behavior on divergence: fail fast with diagnostics.
        if not (
            self.dycp_world_size > 1
        ...
        flag_bools = (gathered_flags > 0.5).flatten()
        has_context_all_ranks = bool(torch.all(flag_bools).item())
        has_context_any_rank = bool(torch.any(flag_bools).item())
        if has_context_any_rank != has_context_all_ranks:
            raise RuntimeError(
                "DyCP chunked-context consensus mismatch across ranks: "
                f"local={int(has_chunked_context_local)} "
                f"all={int(has_context_all_ranks)} "
                f"any={int(has_context_any_rank)} "
                f"flags={flag_bools.to(dtype=torch.int32).tolist()} "
                f"num_dycp_reqs={attn_metadata.num_dycp_reqs} "
    ...
    def _resolve_dycp_prefill_context_branch_consensus(
        ...
        # DyCP prefill-context collectives must be taken in lock-step. Keep
        # behavior strict and fail fast on divergence.
        if not (
            self.dycp_world_size > 1
            and attn_metadata.num_dycp_reqs > 0
        ):
        ...
        gathered_flags = get_dycp_group().all_gather(local_flag, dim=0)
        flag_bools = (gathered_flags > 0.5).flatten()
        can_use_dycp_context_all_ranks = bool(torch.all(flag_bools).item())
        can_use_dycp_context_any_rank = bool(torch.any(flag_bools).item())
        if can_use_dycp_context_any_rank != can_use_dycp_context_all_ranks:
            raise RuntimeError(
                "DyCP prefill-context branch mismatch across ranks: "
                f"local={int(can_use_dycp_context_local)} "
                f"all={int(can_use_dycp_context_all_ranks)} "
                f"any={int(can_use_dycp_context_any_rank)} "
                f"flags={flag_bools.to(dtype=torch.int32).tolist()} "
                f"num_dycp_reqs={attn_metadata.num_dycp_reqs} "
                f"num_prefills={attn_metadata.num_prefills} "
                f"num_decodes={attn_metadata.num_decodes}"
