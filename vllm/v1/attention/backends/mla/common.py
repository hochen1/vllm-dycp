...
class MLACommonImpl(MLACommonBaseImpl[M], Generic[M]):
    ...
    def _resolve_dycp_chunked_context_consensus(
        ...
        """DyCP collectives require all ranks to take the same
        context/no-context branch.  If local metadata diverges, force a
        uniform no-context fallback to avoid cross-rank deadlock."""
        if not (
            self.dycp_world_size > 1
        ...
        return bool(torch.all(gathered_flags > 0.5).item())
    ...
    def _resolve_dycp_prefill_context_branch_consensus(
        ...
        """DyCP prefill-context collectives must be taken by all ranks in
        the DyCP group in lock-step.  If local metadata diverges, force a
        uniform local-context fallback to avoid deadlock."""
        if not (
            self.dycp_world_size > 1
            and attn_metadata.num_dycp_reqs > 0
        ):
