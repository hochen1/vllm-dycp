...
class MLACommonMetadataBuilder(AttentionMetadataBuilder[M]):
    ...
    def build(
        ...
        if num_prefills > 0:
            ...
            # DyCP cross-rank sync: DP-local requests may cause
            # max_context_len_cpu / num_prefills_with_context_cpu to
            # diverge across ranks, making chunked_context metadata
            # inconsistent → NCCL deadlock.  One all_reduce MAX per
            # batch (not per layer) aligns the values.
            if self.dycp_world_size > 1 and prefill_num_dycp_reqs > 0:
                _sync = torch.tensor(
                    [max_context_len_cpu, num_prefills_with_context_cpu],
                    dtype=torch.int64,
                    device=device,
                )
                torch.distributed.all_reduce(
                    ...
                    group=get_dycp_group().device_group,
                )
                max_context_len_cpu = int(_sync[0].item())
                num_prefills_with_context_cpu = int(_sync[1].item())
