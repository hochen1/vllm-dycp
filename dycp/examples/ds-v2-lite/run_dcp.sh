...
args=(
    ...
    --compilation-config '{"cudagraph_capture_sizes":[2, 4, 8, 16, 32, 64, 128, 256, 512, 1024], "cudagraph_mode": "FULL_DECODE_ONLY"}'
    --kv-transfer-config
    '{
...
# ========== execute vllm server ==========
vllm serve "${MODEL_PATH}" "${args[@]}" &> "ds_dcp.log" &
