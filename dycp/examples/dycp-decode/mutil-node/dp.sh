...
if [ $NODE_RANK -eq 0 ]; then
    #curl http://node1-ip:port/dp & \
    curl http://node2-ip:port/dp & \
    #curl http://node3-ip:port/dp & \
    wait
fi
...
vllm serve ${MODEL_PATH} \
    ...
    --compilation-config '{"cudagraph_capture_sizes":[2, 4, 8, 10, 12, 16, 18, 24, 26, 32, 34, 64, 66, 128, 130, 132], "cudagraph_mode": "FULL_DECODE_ONLY"}' \
    --kv-transfer-config \
    '{
